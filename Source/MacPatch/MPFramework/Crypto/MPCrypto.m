...
#define kKID     "MPRSAGen"
#define kKFILE   "/tmp/MPRSAGen.keychain"
...
- (SecKeychainRef)genSecKeychainRef
{
    ...
    //NSLog(@"File: %@",@kKFILE);
    if ([[NSFileManager defaultManager] fileExistsAtPath:@kKFILE]) {
        NSError *error = nil;
        [[NSFileManager defaultManager] removeItemAtPath:@kKFILE error:&error];
        if (error) {
            NSLog(@"%@",error.localizedDescription);
        }
    }
    
    err = SecKeychainCreate(kKFILE, (UInt32)strlen(kid), kid, FALSE, NULL, &keychain);
    
    if (err != noErr) {
        NSLog(@"Error[genSecKeychainRef]: %d",err);
    }
