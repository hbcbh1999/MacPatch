...
def clientMove():
    ...
    groupMember = MpClientGroupMembers().query.filter(MpClientGroupMembers.cuuid == _cuuid).first()
    _group_id_orig = groupMember.group_id
    print _group_id_orig
    
    setattr(groupMember, 'group_id', _group_id)
    db.session.commit()

    return clientGroup(_group_id_orig)
